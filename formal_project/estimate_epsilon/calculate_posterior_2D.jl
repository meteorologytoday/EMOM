...
output_file = "epsilon_logpost2D_$(scheme).nc"

logpost_w = zeros(Float64, Nϵ, Nϵp)
logpost_u = zeros(Float64, Nϵ, Nϵp)
logpost_uw = zeros(Float64, Nϵ, Nϵp)
...
#    end
    ...
    for (i, ϵ) in enumerate(ϵ_vec), (j, ϵp) in enumerate(ϵp_vec)

        #println("[$i, $j] Doing ϵ = $(ϵ); ϵp = $(ϵp)")
        ...
        if scheme == "AGA2020"
            VFX_east   = (   ϵp_sT .* TAUX_east + f_sT      .* TAUY_north  ) .* invD_sT / ρ_sw
...
function processLogpost(logpost, Δϵ, Δϵp)
...
logpost_w, post_w = processLogpost(logpost_w, Δϵ, Δϵp)
logpost_u, post_u = processLogpost(logpost_u, Δϵ, Δϵp)
logpost_uw, post_uw = processLogpost(logpost_uw, Δϵ, Δϵp)
...
Dataset(output_file, "c") do ds
    ...
    defDim(ds, "Nepsp", Nϵp)
    for (varname, vardata, vardim, attrib) in [
        ("logpost_w",  logpost_w,  ("Neps", "Nepsp",), Dict()),
        ("post_w",     post_w,     ("Neps", "Nepsp"), Dict()),
        ("logpost_u",  logpost_u,  ("Neps", "Nepsp",), Dict()),
        ("post_u",     post_u,     ("Neps", "Nepsp"), Dict()),
        ("logpost_uw", logpost_uw, ("Neps", "Nepsp",), Dict()),
        ("post_uw",    post_uw,    ("Neps", "Nepsp"), Dict()),
        ("epsilon",        ϵ_vec,    ("Neps",), Dict()),
        ("epsilon_prime",  ϵp_vec,    ("Nepsp",), Dict()),
    ]
