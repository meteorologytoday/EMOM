...
N   = 11
ϵ_vec = range(0.01, 2.0, length=N) / 86400.0
Δϵ  = ϵ_vec[2] - ϵ_vec[1]
lat_rng = [-20.0, 20.0]
yr_rng = [1, 1]
σ_w = 1e-4  # 10m/day tolerance
σ_u = 1e-2  # 1cm/s   tolerance
N_layer = 5

output_file = "epsilon_logpost_$(scheme).nc"

logpost_w = zeros(Float64, N)
logpost_u = zeros(Float64, N)
...
for (i, ϵ) in enumerate(ϵ_vec)
    ...
    push!(mtx[:ϵ2invβ], ϵ2invβ_sT)
    push!(mtx[:invD],   invD_sT)
    push!(mtx[:ϵ],      ϵ_sT)
end
...
#    end
    ...
    for (k, ϵ) in enumerate(ϵ_vec)

        #println("[$(k)] Doing ϵ = $(ϵ)")
        
        ϵ_sT      = mtx[:ϵ][k]
        invD_sT   = mtx[:invD][k]
        ϵ2invβ_sT = mtx[:ϵ2invβ][k]
        ...
        tmp = - ( Δu / σ_u ).^2.0 / 2.0
        logpost_u[k] += sum( view(tmp, 1, :, :)[valid_idx] )
...
function processLogpost(logpost, Δϵ)
...
logpost_w, post_w = processLogpost(logpost_w, Δϵ)
logpost_u, post_u = processLogpost(logpost_u, Δϵ)
...
Dataset(output_file, "c") do ds
    ...
    defDim(ds, "N", N)
    for (varname, vardata, vardim, attrib) in [
        ("logpost_w",  logpost_w,  ("N",), Dict()),
        ("post_w",     post_w,     ("N",), Dict()),
        ("logpost_u",  logpost_u,  ("N",), Dict()),
        ("post_u",     post_u,     ("N",), Dict()),
        ("epsilon",  ϵ_vec,    ("N",), Dict()),
    ]
